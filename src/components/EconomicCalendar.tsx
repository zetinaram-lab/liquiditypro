// ============================================
// Economic Calendar - High Impact Events + Thermal Optimization
// v1.0.3 - Reduced update frequency to 1s + Page visibility detection
// ============================================
...
// THERMAL OPTIMIZATION: Memoize component
export const EconomicCalendar = memo(({ events }: EconomicCalendarProps) => {
  const { t } = useLanguage();
  const [countdown, setCountdown] = useState<string>('');
  const isPageVisible = usePageVisibility(); // Thermal optimization
  ...
  // Countdown timer effect - THERMAL OPTIMIZATION
  useEffect(() => {
    if (!nextEvent) return;
    
    // Don't run if tab is not visible
    if (!isPageVisible) {
    ...
    updateCountdown();
    // THERMAL OPTIMIZATION: Keep at 1000ms (1s) for countdown precision
    const interval = setInterval(updateCountdown, 1000);

    return () => clearInterval(interval);
  }, [nextEvent, isPageVisible]); // Added isPageVisible dependency
